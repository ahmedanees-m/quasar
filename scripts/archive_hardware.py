"""Fetch the raw data of a completed hardware job and write it to an archive directory.

Writes `raw_counts.json`, `job_metadata.json` and, when available, `backend_properties.json`.
The counts can then be rescored offline with `scripts/rescore_hardware.py`. The IBM Quantum
token is read from ``QISKIT_IBM_TOKEN``.
