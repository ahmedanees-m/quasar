"""G-R.10: Pauli term count of the sparse landscape form against the single-peak projector.

The two forms describe different landscapes. The single-peak projector puts all fitness on one
genotype and has every Z subset in its decomposition, 2^L terms. The additive-plus-epistasis form,
a few site effects and a few pairwise couplings, needs a number of terms linear in L plus one per
coupling. Term counts for NK landscapes of increasing K are recorded alongside.

    python experiments/wp_r/g_r_10_pauli_count.py
"""
...
def run() -> tuple[bool, dict, list[dict]]:
    ...
    for n_sites in SIZES:
        ...
        # The projector's decomposition is known: every Z subset, 2^L of them with the identity
        # absorbed into the mutation identity, plus one transverse term per site. The formula
        # is checked against a built operator up to L = 8.
...
def main() -> int:
    ...
    path = write_gate_record(
        ...
        threshold={
            ...
            "defined_in": "docs/protocol.md, G-R.10",
        },
        measured=measured,
        passed=passed,
        cases=cases,
        notes=(
            "The two forms describe different landscapes. The projector needs every Z "
            "subset; the sparse form needs site terms and one term per coupling. NK term "
            "counts show the growth as K increases."
        ),
    )
