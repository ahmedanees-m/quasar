...
The ground state of the Pauli operator emitted by the compiler is compared with the quasispecies
from the oracle. The compiled operator is also compared entry by entry with the generator
assembled independently in `exact_diag`, since a permutation of the basis can leave the spectrum
unchanged; that comparison is recorded per configuration.

    python experiments/wp_r/g_r_2_hamiltonian_vs_oracle.py
"""
...
def main() -> int:
    ...
    path = write_gate_record(
        ...
        threshold={
            ...
            "defined_in": "docs/protocol.md, G-R.2",
        },
        measured=measured,
        passed=passed,
        cases=cases,
        notes=(
            "The entrywise comparison against the independently assembled generator is "
            "recorded per configuration. Pauli term counts are recorded for G-R.10."
        ),
    )
