...
**A. Splitting exponent.** Trotter evolution is compared with `exp(-H tau)` computed without
splitting, at the same total time from the same initial state, which isolates the splitting
error from the residual of finite tau.

**B. Convergence.** The finest step size, run to a long total time, is scored against the
analytic quasispecies.

    python experiments/wp_r/g_r_3_trotter_scaling.py
"""
...
def run() -> tuple[bool, dict, list[dict]]:
    ...
    for config in CONFIGURATIONS:
        ...
        # B. convergence to the quasispecies at the convergence step size
...
def main() -> int:
    ...
    path = write_gate_record(
        ...
        threshold={
            ...
            "defined_in": "docs/protocol.md, G-R.3",
        },
        measured=measured,
        passed=passed,
        cases=cases,
        notes=(
            "The exponent is fitted against exp(-H tau) computed without splitting. Circuit "
            "depth and two-qubit counts are for the unitary real-time circuit with the same "
            "interaction pattern and are labelled structural, since imaginary-time evolution "
            "is non-unitary. The hardware routes are G-R.6 and G-R.7."
        ),
    )
