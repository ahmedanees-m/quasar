...
Two criteria: cosine >= 0.95 against the reference, looser than for varQITE because the
generator is truncated to a finite support, and no energy increase beyond 1e-10 on any step.

For a real state, Motta's right-hand side ``Re(-i <psi| sigma_I |Delta>)`` vanishes identically
on Pauli strings with an even number of Y factors. The generator basis is therefore the odd-Y
strings, and the right-hand side for both parities is recorded per configuration.

    python experiments/wp_r/g_r_7_motta.py
"""
...
def _descent_report(energies: list[float]) -> dict[str, float | int | bool]:
    """Energy rises larger than ``ENERGY_RISE_TOLERANCE``."""
...
def run() -> tuple[bool, dict, list[dict]]:
    ...
    # Accuracy against generator support at L = 5, K = 2.
    support_scan = []
...
def main() -> int:
    ...
    path = write_gate_record(
        ...
        threshold={
            ...
            "defined_in": "docs/protocol.md, G-R.7",
        },
        measured=measured,
        passed=passed,
        cases=cases,
        notes=(
            "The generator basis is the odd-Y Pauli strings, since a real state needs a real "
            "orthogonal unitary and hence a real antisymmetric generator. The parity block "
            "records Motta's right-hand side for both parities per configuration; the even-Y "
            "set contributes zero. The support scan gives accuracy against generator weight."
        ),
    )
