"""G-R.5: rugged NK landscapes against exact diagonalisation.

On NK landscapes the Pauli decomposition of the fitness is dense. The compiled route goes through
a Walsh-Hadamard decomposition into Pauli terms and back through Qiskit; the reference assembles
the sparse generator directly from the fitness vector.

Trotterised imaginary-time evolution is also run on the L = 8 instances at a fixed imaginary
time and recorded, without a threshold.

    python experiments/wp_r/g_r_5_rugged.py
"""
...
def run() -> tuple[bool, dict, list[dict]]:
    ...
    # Ruggedness statistics against K at L = 8.
...
def main() -> int:
    ...
    path = write_gate_record(
        ...
        threshold={
            ...
            "value": THRESHOLD,
            "defined_in": "docs/protocol.md, G-R.5",
        },
        measured=measured,
        passed=passed,
        cases=cases,
        notes=(
            "The global optimum of an NK landscape sits at a random genotype near Hamming "
            "weight L/2 and is recorded per instance. The Trotterised route is recorded "
            "without a threshold, since an instance with a small gap need not converge at a "
            "fixed imaginary time."
        ),
    )
    ...
    print(f"\nTrotterised imaginary time on {trotter['n_instances']} instances:")
