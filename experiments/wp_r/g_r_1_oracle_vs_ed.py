"""G-R.1: the analytic oracle against exact diagonalisation.

The oracle solves the additive landscape in closed form and permutation-symmetric landscapes by
the Hamming-class reduction. Exact diagonalisation assembles the full 2^L generator and takes its
Perron eigenvector without either structure. The two are compared over every case below.

    python experiments/wp_r/g_r_1_oracle_vs_ed.py
"""
...
def run() -> tuple[bool, dict, list[dict]]:
    ...
    for n_sites in SIZES:
        for mu in MUS:
            ...
            # Family 2: uniform additive, reachable by both analytic routes.
...
def main() -> int:
    ...
    path = write_gate_record(
        ...
        threshold={
            ...
            "value": THRESHOLD,
            "defined_in": "docs/protocol.md, G-R.1",
        },
        measured=measured,
        passed=passed,
        cases=cases,
        notes=(
            "The additive_uniform family is checked by both analytic routes as well as "
            "against exact diagonalisation. Spectral gaps are recorded for every case."
        ),
    )
