...
Two criteria: accuracy against the reference, and identical circuits at two imaginary times.
In varQITE only the parameters move, so each configuration is evolved to tau = 2.5 and tau = 20
and the circuits are compared, before and after transpilation. Transpilation is included because
a rotation left near zero can be optimised away.

    python experiments/wp_r/g_r_6_varqite.py
"""
...
TAU_LONG = 20.0
DTAU = 0.05
# Convergence tolerance on the rate of change of the state. 1e-3 at this step size corresponds to
# a per-step infidelity of about 1e-9.
TOLERANCE = 1e-3
RIDGE = 1e-6
BASIS = ["rz", "sx", "x", "cx"]
HARDWARE_CHECK_MAX_SITES = 4
# Ansatz depth scan at L = 6, K = 2, reported as the worst over three seeds.
REPS_SCAN = {"L": 6, "K": 2, "reps": [4, 6, 8], "seeds": [0, 1, 2]}
# Step sizes for the energy-descent refinement. The continuous flow cannot raise the energy, so a
# rise is explicit Euler overshoot and should shrink with the step. Run at L = 4.
DESCENT_REFINEMENT = {"L": 4, "K": 2, "dtaus": [0.05, 0.02, 0.01, 0.005]}


def reps_for(n_sites: int) -> int:
    """Ansatz depth rule, reps = L + 2."""
    return n_sites + 2
...
def _descent_report(energies: list[float]) -> dict[str, float | int | bool]:
    """Largest energy rise along the evolution, absolute and relative to the total descent.

    The continuous McLachlan flow cannot raise the energy: ``dE/dtau`` equals
    ``-(1/2) grad(E)^T (A + delta I)^-1 grad(E)`` and ``A`` is a Gram matrix. A rise is the
    explicit Euler integrator overshooting at finite step, so its size is recorded.
    """
...
def run() -> tuple[bool, dict, list[dict]]:
    ...
    # Ansatz depth needed at L = 6, worst over three seeds.
    reps_scan = []
...
def main() -> int:
    ...
    path = write_gate_record(
        ...
        threshold={
            ...
            "defined_in": "docs/protocol.md, G-R.6",
        },
        measured=measured,
        passed=passed,
        cases=cases,
        notes=(
            "Ansatz depth rule reps = L + 2; measured.expressibility_scan gives the depth "
            "scan at L = 6. Convergence is judged on the state rather than the parameters, "
            "since the ansatz has gauge directions. tau_used is reported per configuration."
        ),
    )
