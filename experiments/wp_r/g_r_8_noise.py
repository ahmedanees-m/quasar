"""G-R.8: varQITE circuits under simulated device noise, with readout mitigation.

All results use simulated noise models; the hardware run is G-8.

Scoring is in the decoded domain, against the analytic quasispecies. The circuit holds the
quasispecies in its amplitudes, so a computational-basis measurement returns the distribution
squared; at these sizes the two differ by a total-variation distance of 0.22 at cosine 0.987.
`quasarstack.io.conventions.decode_from_measurement` applies the square-root decode.

    python experiments/wp_r/g_r_8_noise.py
"""
    ...
    for n_sites in SIZES:
        for name in ("additive_random", "single_peak"):
            ...
            for device in DEVICES:
                ...
                # Mitigated counts against ideal sampling, in the measured domain.
...
def main() -> int:
    ...
    path = write_gate_record(
        ...
        threshold={
            ...
            "defined_in": "docs/protocol.md, G-R.8",
        },
        measured=measured,
        passed=passed,
        cases=cases,
        notes=(
            "Simulated noise models; the hardware run is G-8. Scoring is in the decoded "
            "domain against the analytic quasispecies, since a computational-basis "
            "measurement returns the squared amplitudes. Total variation is reported beside "
            "cosine; the square-root decode amplifies the noise floor in the tail."
        ),
    )
    ...
    print(f"  max mitigated TV           {measured['max_mitigated_tv']:.4f}")
    print(
        f"  mitigation helped in       {measured['n_cases_mitigation_helped']} of "
        f"{measured['n_cases']} cases"
    )
    print(f"  mitigated vs ideal sampling {measured['min_mitigated_against_ideal_sampling']:.6f}")
