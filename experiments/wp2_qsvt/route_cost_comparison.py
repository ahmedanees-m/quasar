"""Route A against Route B in simulator wall clock and in quantum resources.

Reads the cells of the WP7 quantum sweep in which both routes ran; no new evolution is computed.

* **Route A, varQITE**, runs a shallow parameterised circuit many times inside a classical loop.
  Its cost is circuit repetitions: McLachlan's condition needs the `A` matrix, `n(n+1)/2`
  distinct entries for `n` parameters, and the `C` vector, `n` more, at every imaginary-time
  step. Depth is constant in imaginary time and no ancillas are needed.
* **Route B, QSVT eigenstate filtering**, runs one deep coherent circuit with no optimisation.
  Its cost is query depth: the Chebyshev degree is the number of qubitisation walk operators,
  each two block-encoding calls plus a reflection. It needs `ceil(log2(terms))` ancillas, and
  its accuracy is governed by the normalisation `alpha`.

A state-vector simulator charges for `2^L` arithmetic, so the wall-clock ratio does not describe
hardware cost. The repetition and depth figures are models computed from the recorded `reps`,
`steps` and `degree`, and are labelled as such.
...
def route_a_resources(detail: dict, n_sites: int) -> dict:
    ...
    Parameter count is `L * (reps + 1)`. McLachlan needs the upper triangle of a symmetric
    `n` by `n` Gram matrix plus an `n` vector each step, so the repetition count is
    `steps * (n(n+1)/2 + n)`. Depth is constant in imaginary time (G-R.6).
    """
...
def run() -> tuple[bool, dict, list[dict]]:
    ...
    stream = RESULTS_ROOT / "wp7" / "sweep_full_quantum.jsonl"
...
def main() -> int:
    ...
    path = write_gate_record(
        ...
        cases=cases,
        notes=(
            "The simulator ratio and the resource counts are quoted together. A state-vector "
            "simulator charges for 2^L arithmetic, and Route B touches the state far fewer "
            "times. Route A uses many shallow, ancilla-free circuit repetitions driven by a "
            "classical optimiser; Route B uses one deep coherent circuit of walk-operator "
            "queries plus encoding ancillas. Repetition and depth figures are models computed "
            "from the recorded reps, steps and degree."
        ),
    )
    ...
    print(
        f"  {'cells over the 300 s allocation':32s} "
