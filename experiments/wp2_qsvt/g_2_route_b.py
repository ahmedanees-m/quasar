"""G-2: Route B, QSVT eigenstate filtering for the Hermitian stoquastic operator.

Criteria:

1. Route B reproduces the analytic quasispecies at cosine >= 0.95 for L = 2 to 6.
2. The block encoding satisfies its defining property to 1e-10.
3. The derived polynomial degree agrees with the smallest sufficient degree within a factor of 2,
   using the spectral gap as input.
...
# Verification costs 2**n statevector simulations of an (m + n)-qubit circuit: 15 s for 9 qubits,
# 129 s for 11, and over twenty minutes for 13.
VERIFICATION_QUBIT_BUDGET = 12
# The block-encoding property depends on the construction rather than the coefficient values, so
# two additive seeds per size are verified.
VERIFICATION_SEEDS_PER_FAMILY = 2
MAX_DEGREE = 4096
EPSILON = 1.0 - COSINE_THRESHOLD**2

ROUTE_B_CONSTRUCTION = "QSVT eigenstate filtering for a Hermitian stoquastic operator"


def configurations():
    """Every (label, hamiltonian, fitness) evaluated. Seeded per case."""
...
def run() -> tuple[bool, dict, list[dict]]:
    ...
    for label, operator, fitness in configurations():
        ...
        # alpha is a sum over Pauli coefficients and needs no circuit.
    ...
    measured = {
        "route_b_construction": ROUTE_B_CONSTRUCTION,
...
def main() -> int:
    ...
    path = write_gate_record(
        ...
        threshold={
            ...
            "defined_in": "docs/protocol.md, G-2",
        },
        measured=measured,
        passed=passed,
        cases=cases,
        notes=(
            "The derived degree is the two-factor eigenstate-filtering cost, including the "
            "initial-overlap term, with no fitted constant. It is linear in alpha over the gap: "
            "Chebyshev acceleration to a square root needs the target eigenvalue outside the "
            "encoded spectrum, and here it lies inside."
        ),
    )
