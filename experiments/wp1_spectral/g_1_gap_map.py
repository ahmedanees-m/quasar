"""G-1: the spectral-gap map and three criteria.

The gap governs convergence for every eigenvector-extraction method, quantum or classical. The
map covers single-peak, additive and NK landscapes across the mutation axis.

1. Every closed form is reproduced to relative error < 1e-6.
2. The gap minimum lies within 5% of the analytic mu_c at L = 6, 8, 10, under both readings of
   mu_c: the asymptotic `height / L`, and the susceptibility peak computed from the exact class
   reduction.
3. Every operator-structure claim in `docs/theory.md` resolves to a test or a result file.

The two readings in criterion 2 differ by 29%, 14% and 7% at L = 6, 8, 10 and converge to grid
resolution by L = 24; they are different finite-size locators of a crossover that sharpens with L.
...
def mu_grid(mu_c: float, points: int = GRID_POINTS) -> np.ndarray:
...
def _trusted_class_gap(classes: np.ndarray, mu: float) -> tuple[float, bool]:
    """Class gap, recomputed at 60 digits when the float64 value is below 1e-9.

    Returns ``(gap, used_extended)``.
    """
...
def criterion_two() -> tuple[bool, dict, list[dict]]:
    ...
    # CRITERION_2_SIZES decide the criterion; larger sizes are recorded to show convergence.
    for n_sites in sorted(set(CRITERION_2_SIZES) | {12, 16, 24, 32, 48, 64}):
        for height in PEAK_HEIGHTS:
            ...
            grid = mu_grid(asymptotic)
            gaps = [_trusted_class_gap(classes, mu)[0] for mu in grid]
            mu_gap_grid = float(grid[int(np.argmin(gaps))])

            fine = mu_grid(asymptotic, FINE_GRID_POINTS)
            ...
            deviation_a = abs(mu_gap_grid - asymptotic) / asymptotic
            deviation_b = abs(mu_gap_grid - mu_chi) / mu_chi
            ...
            row = {
                ...
                "grid_spacing_as_fraction_of_mu_c": float((grid[1] - grid[0]) / asymptotic),
...
def criterion_three() -> tuple[bool, dict, list[dict]]:
    ...
    A row whose reference names a missing file, or that gives no reference, is unresolved.
    """
...
def gap_map() -> tuple[dict, list[dict]]:
    ...
    # Whether the symmetric sector holds the second eigenvalue.
    sector = [
    ...
    # Minimum gap at the threshold against L, by arbitrary-precision search. The minimum is an
    # avoided crossing with a slope of order 30 in mu at L = 32, so a 1500-point grid lands well
    # above it; the grid value is recorded alongside for comparison.
    closing = []
    for n_sites in CLASS_SIZES:
        ...
        grid = mu_grid(1.0 / n_sites, FINE_GRID_POINTS)
        grid_minimum = float(
...
def run() -> tuple[bool, dict, list[dict]]:
    """Evaluate the three criteria and the gap map."""
...
def main() -> int:
    ...
    path = write_gate_record(
        ...
        threshold={
            ...
            "defined_in": "docs/protocol.md, G-1",
        },
        measured=measured,
        passed=passed,
        cases=cases,
        notes=(
            "The gap minimum and the susceptibility peak are different finite-size locators "
            "of a crossover that sharpens with L; they converge to grid resolution by L = 24 "
            "and differ by more than 5% at L = 6, 8 and 10. The gap closes where the "
            "population delocalises, as the closing fit shows."
        ),
    )
    ...
    # Criterion 2 rows carry deviation_from_reading_a.
