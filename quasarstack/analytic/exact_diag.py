"""Exact diagonalisation of the full generator.

Assembles

    W = diag(f) + mu * sum_i (X_i - I)

as a ``2**L`` by ``2**L`` sparse matrix from an arbitrary fitness vector and takes its Perron
eigenvector, without using Hamming classes, product states or permutation symmetry. It is the
reference for the closed forms in `quasarstack.analytic.crow_kimura`.

W is symmetric, so the Perron vector is sign definite. Dense diagonalisation is used up to
``dense_limit`` and sparse ``eigsh`` above it; dense operators are refused above L = 12, where
the matrix would need 2.1 GB at L = 14 and 34 GB at L = 16.
"""
...
def perron_vector(
    ...
    """Perron eigenvector of the generator.
    ...
    gap
        ``lambda_1 - lambda_2``, the spectral gap, which sets the conditioning of the Perron
        eigenvector.
    """
    fitness = np.asarray(fitness, dtype=np.float64)
    n_sites = infer_n_sites(fitness)
