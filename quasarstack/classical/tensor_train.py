"""Tensor-train primitives for the imaginary-time cross-check and the operator analysis.

Written in numpy so that each primitive can be checked against dense linear algebra. Baseline C
uses quimb's DMRG (`quasarstack.classical.mps_dmrg`).

Conventions
-----------

A vector of length ``2**L`` becomes ``L`` cores, core ``i`` of shape ``(r[i], 2, r[i+1])`` with
``r[0] = r[L] = 1``. Core ``i`` carries site i, matching the little-endian indexing in
`quasarstack.io.conventions`. ``numpy.reshape`` puts the most significant bit on axis 0, so the
conversion transposes once on the way in and once on the way out.

Rounding and Hadamard products
------------------------------

**Rounding** truncates the train: a right-to-left sweep brings it into left-canonical form, then
a left-to-right sweep truncates singular values. Canonical form is required so that the
discarded singular values are those of the global state rather than of a local core.

**Hadamard product** applies a diagonal operator. The elementwise product of two vectors is the
site-by-site Kronecker product of their cores, which multiplies bond dimensions, so the
operator's bond dimension is the growth factor per step before rounding.
"""
...
def to_tt(
    ...
    ``discarded_weight`` is the total squared singular weight discarded, relative to the squared
    norm.
    """
...
def _svd(matrix: NDArray[np.float64]) -> tuple[NDArray, NDArray, NDArray]:
    """Thin SVD, falling back to LAPACK ``gesvd`` when ``gesdd`` fails to converge.

    ``gesdd`` can fail on numerically rank-deficient matrices, which are common when rounding at a
    bond dimension far above the state's rank. The finiteness check runs first so that NaN or
    infinite input is reported as such.
    """
...
def tt_round(
    ...
    Right-to-left QR to reach left-canonical form, then left-to-right SVD truncation, so that the
    discarded singular values are global.
    """
...
def hadamard(
    ...
    Bond dimensions multiply, so this is followed by `tt_round`.
    """
...
def normalise(cores: list[NDArray[np.float64]]) -> list[NDArray[np.float64]]:
    ...
    Imaginary-time evolution changes the norm by a factor exponential in the number of sites, which
    would overflow a single core.
    """
    scale = norm(cores)
    if scale == 0.0:
