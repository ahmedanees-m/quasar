"""Compile a fitness landscape into a Pauli operator.

The mutation-selection generator is

    W = sum_i a_i Z_i + sum_{i<j} b_ij Z_i Z_j + mu sum_i (X_i - I)

whose Perron eigenvector is the quasispecies. Circuits find ground states, so this module returns
...
``-mu <= 0``, so its ground state is sign definite and L1 and L2 normalisation select the same
ray. The identity term is kept, so the operator matches the generator entry for entry.

Two constructions
-----------------

**Structured.** ``additive_hamiltonian`` writes the diagonal directly as ``a_i Z_i`` and
``b_ij Z_i Z_j``, with ``L + |b| + L + 1`` terms.

**Walsh-Hadamard.** ``diagonal_hamiltonian`` takes an arbitrary fitness vector and recovers its
exact Pauli decomposition, since any diagonal operator expands as

    f = sum_S c_S prod_{i in S} Z_i,    c_S = 2^-L sum_sigma f(sigma) prod_{i in S} z_i

and the inner sum is a Walsh-Hadamard transform. Its term count is the compilation cost of the
landscape; the single-peak projector needs all 2^L terms.

For an additive landscape both constructions give the same operator.

Endianness
----------

Qiskit Pauli strings are little-endian, with the rightmost character on qubit 0. Every term is
built through ``SparsePauliOp.from_sparse_list``, which takes qubit indices explicitly. Qubit i
carries site i, matching `quasarstack.io.conventions`.
"""
...
# Pauli coefficients below this are treated as absent, well under the 1e-9 check tolerances.
...
def diagonal_hamiltonian(fitness: NDArray[np.float64], mu: float) -> SparsePauliOp:
    ...
    The term count reflects how much structure the landscape has.
    """
    fitness = np.asarray(fitness, dtype=np.float64)
...
def pauli_term_count(operator: SparsePauliOp) -> int:
    ...
    The compilation cost of a landscape; G-R.10 compares it between the sparse additive form and the
    single-peak projector.
    """
