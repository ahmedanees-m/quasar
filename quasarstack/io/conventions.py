"""Index, ordering and normalisation conventions.

A wrong bitstring-to-integer convention gives a distribution that is a permutation of the right
one, non-negative and normalised, so every conversion goes through this module.

**Genotype order.** Site 0 is the leftmost locus of the genotype string, as a sequence is
written and as the landscape generators index sites.

**Qiskit order.** Qiskit is little-endian: in a measured bitstring the rightmost character is
qubit 0, so a counts key is reversed before it is read as a genotype.

Qubit i carries site i; only the direction in which a string is read differs.
"""
...
def qiskit_bitstring_to_genotype(bitstring: str) -> str:
    ...
    Qiskit counts keys are little-endian: the rightmost character is qubit 0.
...
def normalise_l1(vector: NDArray[np.float64]) -> NDArray[np.float64]:
    ...
    This is the readout boundary: upstream works in L2, downstream is a probability distribution.
    Taking the modulus is valid because the target is the ground state of a stoquastic operator,
    whose Perron vector is sign definite.
    """
...
def decode_from_measurement(
    ...
    The ground state of the stoquastic Hamiltonian is the Perron vector, which is the quasispecies,
    so the circuit holds the distribution in its amplitudes and a computational-basis measurement
    returns ``|psi|^2``. At L = 3 on an additive landscape the squared distribution is at total
    variation 0.224 from the quasispecies while its cosine similarity is 0.987, which is why total
    variation is reported alongside cosine.

    The square root inverts the squaring in the infinite-shot limit, and a relative error ``eps``
    on a frequency becomes ``eps / 2`` on its square root. A bin with no counts returns exactly
    zero; ``floor`` substitutes a small value for empty bins if wanted.
...
def assert_dense_allowed(n_sites: int, limit: int = 12) -> None:
    """Refuse to build a dense 2^L by 2^L operator above L = 12.

    Dense float64 is about 2.1 GB at L = 14 and 34 GB at L = 16.
    """
    if n_sites > limit:
