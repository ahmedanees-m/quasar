...
| C30 | The G-7 null does not depend on the budget exclusion rule. The tensor network overruns its allotment on 64.1% of `L = 12` cells, and the rule removes exactly the cells where the classical reference is most strained. Scored both ways: with exclusions the worst tensor-network cosine is 0.999981, without them it is 0.875797, and **zero** cells fall below the 0.80 threshold either way | G-7 | `results/_local/g_7_budget_sensitivity.json` | `scripts/budget_sensitivity.py` | pass |
