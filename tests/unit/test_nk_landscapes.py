"""NK landscapes and ruggedness statistics.

Standardisation makes K vary ruggedness without changing selection strength, and the statistics
report where the optimum sits.
"""
...
def test_standardisation_fixes_the_selection_strength() -> None:
    """Raw NK spread shrinks as 1/sqrt(L) and grows with K, so fitness is standardised."""
    for n_sites in (6, 8):
        for k in (0, 2, 5):
...
def test_k_zero_is_additive(n_sites: int) -> None:
    """With no epistatic partners the Pauli decomposition has weight-one support only: L
    longitudinal terms, L transverse terms and an identity.
    """
