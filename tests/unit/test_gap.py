"""Spectral gap: closed forms, the symmetric sector, and extended precision."""
...
def test_additive_gap_does_not_close_with_system_size() -> None:
    """The additive gap is independent of L for the same per-site fitness."""
...
def test_lambda_two_of_the_additive_generator_is_l_fold_degenerate() -> None:
    """The second eigenvalue flips the cheapest single site; with equal sites there are L
    equivalent choices, which is why the gap is L-independent.
    """
    n_sites = 8
...
def test_gap_saturates_at_twice_mu_above_the_error_threshold(n_sites: int) -> None:
    """Above the error threshold the gap approaches 2 mu as L grows, so the tolerance is
    looser at small L.
    """
...
def test_class_reduction_finds_the_true_second_eigenvalue_for_the_single_peak(
    ...
    """For the single peak, ``lambda_2`` lies in the symmetric sector, so the class gap is
    the full gap. This does not hold by construction.
    """
...
def test_symmetric_sector_gap_can_only_exceed_the_full_gap() -> None:
    """Restricting to a sector can only remove eigenvalues between lambda_1 and lambda_2."""
...
def test_class_tridiagonal_agrees_with_the_quasispecies_construction() -> None:
    """Same matrix as `crow_kimura.class_quasispecies` builds internally."""
...
def test_extended_precision_agrees_with_float64_where_float64_is_trustworthy() -> None:
    """Away from the avoided crossing, Sturm bisection agrees with float64."""
...
def test_the_threshold_sits_at_mu_times_l_equal_to_the_peak_height() -> None:
    ...
    ``mu* L / height`` agrees between two heights to the precision of the search.
...
def test_extended_precision_needs_no_dependency_outside_the_standard_library() -> None:
    """The extended-precision path imports nothing outside the standard library."""
    import ast
