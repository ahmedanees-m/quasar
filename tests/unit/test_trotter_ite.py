...
The propagator is non-unitary. The tests check that each module is the intended operator, that
the splitting is second order, and that the structural circuit is labelled as a unitary analogue.
"""
...
def test_module_m_is_the_transverse_field_exponential(n_sites: int) -> None:
    ...
    for site in range(n_sites):
        ...
        # reorder so that bit 0 is the fastest-varying index
        operator = np.array([[1.0]])
...
def test_module_s_is_a_diagonal_scaling_up_to_a_positive_constant() -> None:
    """apply_selection shifts the exponent by its maximum, which multiplies the result by a
    positive scalar, so the check is on direction.
    """
...
def test_step_count_must_divide_the_total_time() -> None:
    """A step size that does not divide tau is rejected, so every step size in a scaling
    sweep reaches the same total time.
    """
...
def test_splitting_error_falls_by_four_when_the_step_is_halved() -> None:
    """Second order, checked directly through the error ratio."""
...
def test_without_mutation_selection_collapses_onto_the_fittest_genotype() -> None:
    """Module S alone collapses onto the fittest genotype.

    The runner-up is one flip away at the weakest site and keeps weight ``exp(-2 * 0.5 * tau)``,
    about 1e-17 at tau = 40.
...
def test_structural_circuit_is_the_unitary_analogue_and_is_labelled_as_such() -> None:
    """The structural circuit is a unitary circuit with the same interaction pattern, used
...
def test_structural_circuit_depth_grows_linearly_in_size() -> None:
    """The additive case has no two-qubit gates, so depth does not grow with L beyond the
