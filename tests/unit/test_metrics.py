"""Scoring metrics: cosine similarity and total-variation distance."""
...
def test_total_variation_detects_a_wrong_tail() -> None:
    """Two distributions that agree on the mass but differ by an order of magnitude across the
    tail: cosine barely moves, total variation does.
    """
    p = np.zeros(64)
    p[0] = 0.9
    p[1:] = 0.1 / 63
