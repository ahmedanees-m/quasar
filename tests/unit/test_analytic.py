"""The analytic oracle and the exact-diagonalisation reference.

Gate G-R.1 is the full comparison. These are fast checks: closed forms against hand-computed
values, limits, and small-L agreement between the two routes.
"""
...
def test_the_two_analytic_routes_agree_where_they_overlap(
    ...
    """Uniform additive fitness is solvable by both analytic routes, which must agree."""
...
def test_generator_rows_sum_to_the_fitness_alone() -> None:
    """Mutation conserves probability, so each row sums to the fitness."""
