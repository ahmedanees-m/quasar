"""Reproducibility of the sparse eigensolver.

`scipy.sparse.linalg.eigsh` starts from NumPy's global random state unless given a start vector,
so results can differ in the last digits between processes. These tests fail if any `eigsh` call
in `quasarstack` omits a fixed start vector.
"""
...
def test_start_vector_is_the_same_every_call() -> None:
    ...
    np.random.seed(12345)  # noqa: NPY002 - disturbing the global state
...
def test_sparse_gap_is_bit_identical_across_global_rng_states(n_sites: int) -> None:
    """The sparse gap is bit-identical whatever the global random state."""
...
def test_no_eigsh_call_in_the_package_omits_its_start_vector() -> None:
    """No `eigsh` call in the package omits `v0`."""
    offenders = []
