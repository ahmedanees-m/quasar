...
The locator is checked on curves whose answer is known by construction, including curves with no
interior transition.
"""
...
def test_uniform_population_has_zero_surplus() -> None:
    """A uniform population has zero surplus."""
...
def test_locator_reports_when_no_interior_peak_exists() -> None:
    ...
    zero mutation rate, so there is no interior threshold and the flag reports that.
    """
...
def test_sharp_peak_does_have_an_interior_threshold() -> None:
    """The single peak has an interior threshold."""
    mus = np.round(np.arange(0.01, 2.001, 0.01), 10)
...
def test_synergistic_coupling_holds_the_population_together_to_higher_mutation() -> None:
    """Synergistic coupling moves the half-surplus crossover to higher mutation rate at L = 8.

    G-R.4 records this across sizes. The test fixes L = 8 and the mean-field coupling scale,
    since at fixed coupling the total interaction grows as L squared.
