"""Noise models, the sampling pipeline, and the square-root decode.

Endianness is checked through transpilation and sampling, since a permuted distribution is still
a valid distribution. The square-root decode inverts the amplitude encoding; the undecoded
measurement scores 0.987 on cosine.
"""
...
def test_connectivity_differs_between_the_device_classes() -> None:
    """Restricted connectivity forces swaps, which add two-qubit gates."""
...
def test_the_superconducting_model_pays_for_connectivity_it_does_not_have() -> None:
    """Restricted connectivity costs two-qubit gates.

    A star, where one qubit couples to every other, cannot be embedded in a chain without
    routing. A single long-range gate does not show this, since with two active qubits the
    transpiler places them adjacently.
...
def test_endianness_survives_the_whole_pipeline(genotype: str) -> None:
    """A basis state survives preparation, transpilation, sampling and decoding.

    A wrong bitstring convention anywhere along the path gives a permuted distribution that is
    non-negative, normalised, and on the wrong genotype.
...
def test_the_measured_assignment_matrix_is_nearly_the_identity() -> None:
    """Calibration is diagonally dominant. It is measured from circuits, so it carries shot
    noise.
    """
...
def test_mitigation_returns_a_genuine_distribution() -> None:
    """A plain inverse returns negative entries once shot noise is comparable to the
    correction, so the solve is constrained rather than clipped.
    """
...
def test_the_decode_inverts_squaring() -> None:
    """The circuit holds the distribution in its amplitudes, so a measurement returns it
    squared.
    """
...
def test_the_undecoded_measurement_differs_in_total_variation() -> None:
    """A concentrated distribution and its square are close on cosine and far apart in
    total variation.
    """
