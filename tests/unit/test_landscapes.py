"""Landscape construction against hand-computed values.

A projector-convention slip changes every fitness by a constant and a factor, which shifts the
quasispecies without an obvious sign of error.
"""
...
def test_uniform_additive_matches_the_class_form(n_sites: int, a_value: float) -> None:
    """The two constructions of the uniform additive landscape agree entry by entry, which
    lets G-R.1 compare the closed form with the Hamming-class reduction.
    """
