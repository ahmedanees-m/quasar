...
Checks the sign and shift convention, under which the compiler emits ``-W`` including the
identity term so that its matrix cancels the generator exactly, and the endianness, which
spectral checks cannot detect because permuting the basis leaves the spectrum unchanged.
"""
...
def test_qubit_i_carries_site_i() -> None:
    """Qubit i carries site i.

    A strongly positive coefficient on site 0 pushes mass onto genotypes whose site 0 is wild
    type. Reversed Pauli strings would put it on the last site with an unchanged spectrum.
...
def test_compiled_operator_is_minus_the_generator() -> None:
    """Entry by entry, including the identity term."""
...
def test_hamiltonian_is_stoquastic() -> None:
    """Every off-diagonal entry is non-positive, so the ground state has a definite sign and
    the L1 decode applies.
    """
...
def test_the_two_compiler_routes_agree_on_additive_landscapes(n_sites: int) -> None:
    """The structured build and the Walsh-Hadamard build give the same operator."""
...
def test_single_peak_projector_needs_every_z_subset() -> None:
    """The single peak needs all 2^L Z subsets, the identity merging with the mutation
    identity, plus L transverse terms. This is the numerator of the G-R.10 ratio.
    """
