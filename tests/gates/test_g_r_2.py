"""Gate G-R.2 as a test, under the `gate` marker."""

from __future__ import annotations

import pytest

from experiments.wp_r.g_r_2_hamiltonian_vs_oracle import (
...
def test_g_r_2_hamiltonian_ground_state_matches_the_oracle() -> None:
    ...
    assert (
        len(cases) == REQUIRED_CONFIGURATIONS
    ), f"G-R.2 requires {REQUIRED_CONFIGURATIONS} configurations, the run produced {len(cases)}"
    assert passed, (
